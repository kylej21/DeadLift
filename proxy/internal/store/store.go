...
func (s *Store) CreateBatch(ctx context.Context, batch models.Batch) error {
	_, err := s.fs.Collection("batches").Doc(batch.BatchID).Set(ctx, batch)
	return err
}

func (s *Store) GetBatch(ctx context.Context, batchID string) (*models.Batch, error) {
	doc, err := s.fs.Collection("batches").Doc(batchID).Get(ctx)
	if err != nil {
...
// GetPendingBatchBySubscription returns the open pending batch for the given org+subscription, or nil if none.
func (s *Store) GetPendingBatchBySubscription(ctx context.Context, orgID, subscription string) (*models.Batch, error) {
	docs, err := s.fs.Collection("batches").
		Where("org_id", "==", orgID).
	...
	var batch models.Batch
	if err := docs[0].DataTo(&batch); err != nil {
		return nil, fmt.Errorf("parse batch: %w", err)
	}
	return &batch, nil
}

func (s *Store) AddTaskToBatch(ctx context.Context, batchID, taskID string) error {
	_, err := s.fs.Collection("batches").Doc(batchID).Update(ctx, []firestore.Update{
		{Path: "task_ids", Value: firestore.ArrayUnion(taskID)},
		{Path: "task_count", Value: firestore.Increment(1)},
		{Path: "updated_at", Value: time.Now()},
	})
	return err
}
...
func (s *Store) ListBatchesByOrg(ctx context.Context, orgID string) ([]models.Batch, error) {
	docs, err := s.fs.Collection("batches").Where("org_id", "==", orgID).Documents(ctx).GetAll()
	if err != nil {
		return nil, fmt.Errorf("list batches: %w", err)
	}
	batches := make([]models.Batch, 0, len(docs))
	for _, doc := range docs {
		var b models.Batch
	...
	for i := 0; i < len(batches)-1; i++ {
		for j := i + 1; j < len(batches); j++ {
			if batches[j].FirstSeen.After(batches[i].FirstSeen) {
...
func (s *Store) GetTasksByBatch(ctx context.Context, batchID string) ([]models.Task, error) {
	docs, err := s.fs.Collection("tasks").Where("batch_id", "==", batchID).Documents(ctx).GetAll()
	if err != nil {
		return nil, fmt.Errorf("get tasks by batch: %w", err)
	}
	tasks := make([]models.Task, 0, len(docs))
	for _, doc := range docs {
		var t models.Task
...
func (s *Store) UpdateTaskBatchID(ctx context.Context, taskID, batchID string) error {
	_, err := s.fs.Collection("tasks").Doc(taskID).Update(ctx, []firestore.Update{
